...
void uppergoingtask(void const * argument)
{
  ...
  */
  ...
  float x_qu = 196.0;
  float x_qu_ps = 600;
  float x_fang_4 = 3033.0;//3060  3040
  float x_fang_2 = 2780.0;
  float x_middle = 1769.0;
  float x_zhan = 2560.0;  
    ...
    if(jiaquzhuangtai == 0)//夹取1和4的箱子
    {
      ...
      if(runflag == 4)//取了第一个箱子
      {
        ...
        if(diff < 4)
        {
          ...
          pid_reset(&(mygantry.Motor_X->speedPID), 1.8, 0.003, 0.01);
      ...
      else if(mapping[0]==0 && mapping[3]!=0) //先放[3]又因为[3]不是0 可以先放
      {
        ...
        if(step01 == 4)
        {
          ...
          if(diff < 4)
          {
            ...
            __HAL_TIM_SetCompare(&htim4,TIM_CHANNEL_1,zhuazicaiclose);
            osDelay(500);
            pid_reset(&(mygantry.Motor_X->speedPID), 1.8, 0.01, 0.001);
    ...
    if(jiaquzhuangtai == 3 )
    {
      if(mapping[1]!=0 && mapping[4]!=0)
      {
        ...
        if(step10 == 200)
        {
          ...
          if(mapping[1] == 2) motor_controlmode(&mi_motor[0], 0, S0+errorth, 0, 1.8, 1.0);
    ...
    if(jiaquzhuangtai == 5 )
    {
      if(mapping[2]!=0 && mapping[5]!=0)
      {
        ...
        if(step20 == 200)
        {
          ...
          if(mapping[2] == 2) motor_controlmode(&mi_motor[0], 0, S0+errorth, 0, 1.8, 1.0);
        ...
        if(step20 == 3)
        {
          if(mapping[2] < 6 && mapping[2] > 0)//不需要转动直接进行放置再回到中点
          {
            //转180度
            mygantry.gantrypos.x = x_fang_4;
            switch(mapping[2]){
